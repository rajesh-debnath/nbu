...
<head>
	...
	<title>Alumni Applicant Pending</title>
...
<div class="table-sm pt-2 pb-2 m-2 "><!--main page-->
  ...
  <?php 
  $sql = "SELECT alum_applicant.* FROM alum_applicant WHERE al_reg_no NOT IN (SELECT alum_profile.alp_reg_no FROM alum_profile)";
  ...
  while($al_profile = mysqli_fetch_array($result)) { 
   ...
   <tbody>
      <tr>
       ...
       <td><small><?php echo $al_profile['al_name']; ?></small></td>
       <td><small><?php echo $al_profile['al_reg_no']; ?></small></td>
       <td><small><?php echo $al_profile['al_course']; ?></small></td>
       <td><small><a href="<?php echo $baseurl;?>admin_al_profile.php?alum_reg_no=<?php echo $al_profile['al_reg_no'];?>"><button class="btn-secondary">view</button></a></small></td>
